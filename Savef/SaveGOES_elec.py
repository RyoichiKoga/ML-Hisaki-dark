...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 17 16:47:53 2025
...
time1, data = get_data('g15_epead_E1E_COR_FLUX')
time2, flag = get_data('g15_epead_E1E_QUAL_FLAG')

#Mask the invalid data
initial_mask = (flag == 0)
valid_value_mask = (data > -10000)
mask = initial_mask & valid_value_mask
...
if len(args) == 4:
    if os.path.exists(args[3]) and os.path.isdir(args[3]):
        filename = os.path.join(args[3], f"GOES_elec_{year}")
    else:
        print("Directory does not exist")
        sys.exit(1)
else:
    filename = f"GOES_elec_{year}"
