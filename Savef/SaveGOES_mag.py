...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 18 17:37:22 2025

@author: Ryoichi Koga
"""
# Example to run this program
#%run saveGOES_**.py '2018-08-01' '2018-08-31' 'directry (option)'
import pyspedas
...
# Data acquistion（GOES-15 magnetometer）
mag_vars = pyspedas.goes.fgm(trange=[args[1], args[2]], probe='15',
                             instrument='fgm', datatype='1min')

time1, data = get_data('g15_fgm_HP_1')
time2, flag = get_data('g15_fgm_HP_1_QUAL_FLAG') 

# Mask the invalid data：flag == 0 and flux > -10000
mask = (flag == 0) & (data > -10000)# & (data < 500) #Anomaly spike up to 1000
...
plt.ylabel('Magnetic Field (HP component)')
plt.title(f'GOES-15 Magnetic Field HP in {year}')
plt.grid(True)
plt.tight_layout()
plt.show()
...
if len(args) == 4:
    if os.path.exists(args[3]) and os.path.isdir(args[3]):
        filename = os.path.join(args[3], f"GOES_mag_{year}")
    else:
        print("Directory does not exist")
        sys.exit(1)
else:
    filename = f"GOES_mag_{year}"
