...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 18 17:19:46 2025
...
mag_vars = pyspedas.goes.fgm(trange=[args[1], args[2]], probe='15',
                             instrument='epead', datatype='5min')

time1, data = get_data('g15_epead_P1E_COR_FLUX')
time2, flag = get_data('g15_epead_P1E_QUAL_FLAG')

# Mask：flag == 0 and flux > -10000
...
plt.ylabel('Proton Flux (P1E COR)')
plt.title(f'GOES-15 Proton Flux in {year}')
plt.grid(True)
plt.tight_layout()
plt.show()
...
for idx in gap_indices:
    print(f"Decect detection: DOY {DOY[idx]} → {DOY[idx+1]} (Difference: {delta_days[idx]}day)")

# Construct the output filename based on the year
if len(args) == 4:
    if os.path.exists(args[3]) and os.path.isdir(args[3]):
        filename = os.path.join(args[3], f"GOES_proton_{year}")
    else:
        print("Directory does not exist")
        sys.exit(1)
else:
    filename = f"GOES_proton_{year}"
